...
export default function Header(props) {
  ...
  let dataTagline, dataIcon;

  const { data: logoicon, isSuccess } = useLogoIcon();

  if(isSuccess) {
    const dataList = logoicon.filter(list => list._id === "62a8a745240f8329a43c7646");
  ...
  return (
    <AppBar position="fixed" className={classes.appBar}>
      ...
      <Toolbar className={classes.toolbar}>
        ...
        >
          <Avatar className={classes.orange}>{user.result.name.charAt(0)}</Avatar>
        </IconButton>
        <Menu
          id="mail-menu"
        ...
        >
          <div className={classes.profileMenuUser}>
            <Typography align="center" variant="h4" weight="medium">
              {user.result.name}
            </Typography>
